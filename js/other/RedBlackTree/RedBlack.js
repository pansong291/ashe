...
RedBlack.prototype.addControls = function () {
    ...
    this.insertField.onkeydown = this.returnSubmit(this.insertField, this.insertCallback.bind(this), 8);
    this.insertButton = addControlToAlgorithmBar("Button", "插入/Insert");
    this.insertButton.onclick = this.insertCallback.bind(this);
    this.deleteField = addControlToAlgorithmBar("Text", "");
    this.deleteField.onkeydown = this.returnSubmit(this.deleteField, this.deleteCallback.bind(this), 8);
    this.deleteButton = addControlToAlgorithmBar("Button", "删除/Delete");
    this.deleteButton.onclick = this.deleteCallback.bind(this);
    this.findField = addControlToAlgorithmBar("Text", "");
    this.findField.onkeydown = this.returnSubmit(this.findField, this.findCallback.bind(this), 8);
    this.findButton = addControlToAlgorithmBar("Button", "查找/Find");
    this.findButton.onclick = this.findCallback.bind(this);
    this.printButton = addControlToAlgorithmBar("Button", "遍历/Print");
    this.printButton.onclick = this.printCallback.bind(this);
    this.showNullLeaves = addCheckboxToAlgorithmBar("显示nil叶子结点/Show Null Leaves");
...
RedBlack.prototype.insertElement = function (insertedValue) {
    ...
    this.cmd("SetText", 0, " 插入/Inserting " + insertedValue);
    this.highlightID = this.nextIndex++;
    var treeNodeID;
    if (this.treeRoot == null) {
        treeNodeID = this.nextIndex++;
...
RedBlack.prototype.singleRotateRight = function (tree) {
    ...
    var t1 = A.left;
    var t2 = A.right;
    this.cmd("SetText", 0, "右旋/Single Rotate Right");
...
RedBlack.prototype.singleRotateLeft = function (tree) {
    ...
    var t2 = B.left;
    var t3 = B.right;
    this.cmd("SetText", 0, "左旋/Single Rotate Left");
...
RedBlack.prototype.insert = function (elem, tree) {
    ...
    if (elem.data < tree.data) {
        this.cmd("SetText", 0, elem.data + " < " + tree.data + ".  看左子树/Looking at left subtree")
    } else {
        this.cmd("SetText", 0, elem.data + " >= " + tree.data + ".  看右子树/Looking at right subtree")
    }
    this.cmd("Step");
    this.cmd("SetHighlight", tree.graphicID, 0);
    ...
    if (elem.data < tree.data) {
        if (tree.left == null || tree.left.phantomLeaf) {
            this.cmd("SetText", 0, "发现空树或叶子节点，插入元素/Found null tree (or phantom leaf), inserting element");
            if (tree.left != null) {
...
RedBlack.prototype.fixDoubleRed = function (tree) {
    if (tree.parent != null) {
        ...
        if (tree.parent.parent == null) {
            this.cmd("SetText", 0, "根节点是红色的，应该修改为黑色/Tree root is red, color it black.");
            this.cmd("Step");
        ...
        if (this.blackLevel(uncle) == 0) {
            this.cmd("SetText", 0, "节点和父节点都是红色，叔叔节点也是红色（从节点祖父节点推算得到）/Node and parent are both red.  Uncle of node is red （push blackness down from grandparent）");
            this.cmd("Step");
            ...
            if (tree.isLeftChild() && !tree.parent.isLeftChild()) {
                this.cmd("SetText", 0, "节点和父节点都是红色。节点左子节点，父节点是右子节点，旋转/Node and parent are both red.  Node is left child, parent is right child -- rotate");
                this.cmd("Step");
                this.singleRotateRight(tree.parent);
                tree = tree.right
            } else {
                if (!tree.isLeftChild() && tree.parent.isLeftChild()) {
                    this.cmd("SetText", 0, "节点和父节点都是红色。节点右子节点，父节点是左色，旋转/Node and parent are both red.  Node is right child, parent is left child -- rotate");
                    this.cmd("Step");
            ...
            if (tree.isLeftChild()) {
                this.cmd("SetText", 0, "节点和父节点都是红色的。结点是左子结点，父结点是左子结点/Node and parent are both red.  Node is left child, parent is left child\n需要进行旋转/Can fix extra redness with a single rotation");
                this.cmd("Step");
                ...
                this.cmd("SetText", 0, "节点和父节点都是红色的。结点是右子结点，父结点是右子结点/Node and parent are both red.  Node is right child, parent is right child\n需要进行旋转/Can fix extra redness with a single rotation");
                this.cmd("Step");
        ...
        if (tree.blackLevel == 0) {
            this.cmd("SetText", 0, "根节点是红色的，应该修改为黑色/Root of the tree is red.  Color it black");
            this.cmd("Step");
            tree.blackLevel = 1;
...
RedBlack.prototype.fixLeftNull = function (tree) {
    var treeNodeID = this.nextIndex++;
    var nullLeaf;
    this.cmd("SetText", 0, "将叶子节点变成黑色/Coloring 'Null Leaf' double black");
    this.cmd("CreateCircle", treeNodeID, "NULL\nLEAF", tree.x, tree.y);
    this.cmd("SetForegroundColor", treeNodeID, FOREGROUND_BLACK);
...
RedBlack.prototype.fixExtraBlackChild = function (parNode, isLeftChild) {
    ...
    if (this.blackLevel(sibling) > 0 && this.blackLevel(sibling.left) > 0 && this.blackLevel(sibling.right) > 0) {
        ...
        if (this.blackLevel(sibling) == 0) {
            ...
            if (isLeftChild && this.blackLevel(sibling.right) > 0) {
                ...
                if (!isLeftChild && this.blackLevel(sibling.left) > 0) {
...
RedBlack.prototype.treeDelete = function (tree, valueToDelete) {
    ...
    if (tree != null && !tree.phantomLeaf) {
        ...
        if (valueToDelete < tree.data) {
            this.cmd("SetText", 0, valueToDelete + " < " + tree.data + ".  在左子树中查找/Looking at left subtree")
        } else {
            if (valueToDelete > tree.data) {
                this.cmd("SetText", 0, valueToDelete + " > " + tree.data + ".  在右子树中查找/Looking at right subtree")
            } else {
                this.cmd("SetText", 0, valueToDelete + " == " + tree.data + ".  找到要删除的节点/Found node to delete")
            }
        }
        this.cmd("Step");
        this.cmd("SetHighlight", tree.graphicID, 0);
        if (valueToDelete == tree.data) {
            ...
            if (((tree.left == null) || tree.left.phantomLeaf) && ((tree.right == null) || tree.right.phantomLeaf)) {
                this.cmd("SetText", 0, "要删除的节点是一个叶子结点。删除它。/Node to delete is a leaf.  Delete it.");
...
RedBlack.prototype.resizeTree = function () {
    ...
    if (this.treeRoot != null) {
        if (this.treeRoot.leftWidth > startingPoint) {
            ...
            if (this.treeRoot.rightWidth > startingPoint) {
