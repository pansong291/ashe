...
;(function () {
  let Utils = {
    ...
    isRGB: function isRGB(value) {
      let rgb = value.match(this.reRGB)
    ...
    isHSL: function isHSL(value) {
      let hsl = value.match(this.reHSL)
    ...
    mix: function mix(color1, color2, percentage) {
      ...
      let r = Math.round(color1.rgb.r * w1 + color2.rgb.r * w2)
      let g = Math.round(color1.rgb.g * w1 + color2.rgb.g * w2)
      let b = Math.round(color1.rgb.b * w1 + color2.rgb.b * w2)
  ...
  Values.prototype.tints = function tint(percentage) {
    let i = (percentage = percentage || 10),
      tints = []
  ...
  Values.prototype.all = function all(percentage) {
    ...
    let tints = this.tints(percentage).reverse()
    let shades = this.shades(percentage)
    tints.push(this)
  ...
  Values.prototype.getBrightness = function getBrightness() {
    let sum = this.rgb.r + this.rgb.g + this.rgb.b
  ...
  Values.prototype._setFromHSLString = function _setFromHSLString(color) {
    let hsl = color.match(Utils.reHSL),
      h = Math.round(hsl[1]),
      s = Math.round(hsl[2]),
      l = Math.round(hsl[3])
